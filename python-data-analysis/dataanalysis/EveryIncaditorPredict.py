...
def inverse_transform_col(scaler,y,n_col):
    ...
    return y
    # feature_min = scaler.min_[n_col]
    # feature_scale = scaler.scale_[n_col]
    # y_retransformed = y.copy()
    # y_retransformed *= feature_scale
    # y_retransformed += feature_min
    # return y_retransformed
    # elif type(scaler) == StandardScaler:
    # feature_mean = scaler.mean_[n_col]
    # feature_std = scaler.scale_[n_col]
    # y_retransformed = y.copy()
    # y_retransformed *= feature_std
    # y_retransformed += feature_mean
    # return y_retransformed
    # # else:
    #     raise Exception("Scaler should be either MinMaxScaler or StandardScaler!")
...
def prepareData():
    ...
    return predictData
def IndicatorPrediction(indicator):
    data = pd.read_csv("data\province.csv")
    df = data[["行政区划代码", "年份", indicator]]

    df = df[df["年份"] >= 2000]
    inputData = df.reset_index()
    predictData = inputData[["行政区划代码", "年份", "地区生产总值"]]
    li = predictData.groupby("行政区划代码").count().reset_index()
    for i in range(31):
        year = 2020
    ...
    # min_max_scaler = MinMaxScaler()
    df0 = min_max_scaler.fit_transform(predictData.interpolate())
    inputData = pd.DataFrame(df0, columns=predictData.columns)
    ...
    y_pre=y_pre[:,0]
    # y_pre = min_max_scaler.fit_transform(y_pre)

    y_pre=inverse_transform_col(min_max_scaler,y_pre,n_col=0)      # 对预测值反归一化
    y_train=inverse_transform_col(min_max_scaler,y_train,n_col=0)    # 对实际值反归一化（如果不想用，这两行删除即可）
    #画图 #展示在测试集上的表现
    print(y_pre)
    xdata = range(1,652)
    xxdata = range(652,963)
    realData = [list(i) for i in zip(xdata,y_train)]
    predictData = [list(i) for i in zip(xxdata,y_pre)]
    dictPredict = {
        "realData":realData,
        "predictData":predictData
    }

    return json.dumps(dictPredict)

#
# if __name__ == '__main__':
#     print(IndicatorPrediction("地区生产总值"))
