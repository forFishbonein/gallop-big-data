...
def read_check_data(pro):
    ...
    data = mycol.find({},{"_id": 0, '地区': 1, '年份': 1,
                                    '第一产业增加值(万元)': 1,'第二产业增加值(万元)':1,"第三产业增加值(万元)":1,"地区生产总值(万元)":1})
    ...
    x_first_train = np.array(data['第一产业增加值(万元)'].loc[:15])

    x_second_train = np.array(data['第二产业增加值(万元)'].loc[:15])
    x_third_train = np.array(data['第三产业增加值(万元)'].loc[:15])
    x = list(data["年份"])
    y_income = np.array(data['地区生产总值(万元)'].loc[:15])
    x_first_predict = np.array(data['第一产业增加值(万元)'])
    x_second_predict = np.array(data['第二产业增加值(万元)'])
    x_third_predict = np.array(data['第三产业增加值(万元)'])
    # 可视化展示数据
