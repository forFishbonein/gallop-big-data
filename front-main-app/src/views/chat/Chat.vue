...
const clickToSend = () => {
  if (prompt.value) {
    if (requestingFlag === false) {
      ...
      const data: promptType = { message: prompt.value };
      ...
      // sendMessage(data)
      //TODO 这里为了区分接口路径
      axios.post("http://localhost:8080/chatbot/talk", data).then((res) => {
        outputText(res.data);
        myHistory.value.dialogues.push({
    ...
    $(document).ready(function () {
      $(".change-model-button").on("click", function () {
        ...
        if ($("#main-container").css("justify-content") == "left") {
          ...
          $(".history-wapper").css("border", "1px transparent solid");
        } else {
          // @ts-ignore
          $(this).html(
          ...
          $(".history-wapper").css("width", "20%");
          $(".history-wapper").css("border", "1px #dcdfe6 solid");
          setTimeout(() => {
